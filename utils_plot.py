...
def plot_supply_scatter(year, df_plot, df_demand):
    ...
    def get_step_xy(df):
        ...
        for i in range(len(df) - 1):
            # 垂直線
            x_vals += [df["累積容量"].iloc[i], df["累積容量"].iloc[i]]
            y_vals += [df["成本_萬元"].iloc[i], df["成本_萬元"].iloc[i+1]]
            
            # 橫線段
            x_vals += [df["累積容量"].iloc[i], df["累積容量"].iloc[i+1]]
            y_vals += [df["成本_萬元"].iloc[i+1], df["成本_萬元"].iloc[i+1]]
    ...
    for i in range(len(df_plot) - 1):
        if df_plot["累積容量"].iloc[i] <= df_demand < df_plot["累積容量"].iloc[i + 1]:
            ...
            hover_price = df_plot["成本_萬元"].iloc[i+1]
            site = str(df_plot["電廠"].iloc[i+1] or "")
            num = str(df_plot["機組"].iloc[i+1] or "")
            label = f"需求交點<br>容量: {df_demand:.0f} MW<br>價格: {hover_price:.2f} 萬元<br>機組: {site + num}"
            break
    else:
